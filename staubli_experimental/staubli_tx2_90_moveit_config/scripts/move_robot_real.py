...
def move_robot():
    ...
    group.set_max_velocity_scaling_factor(0.2)  # Escala de velocidad para hardware real
    group.set_max_acceleration_scaling_factor(0.2)  # Escala de aceleración
    ...
    target_pose_1.position.y = start_pose.position.y - 0.1
    ...
    target_pose_3.position.x = target_pose_2.position.x - 0.1
    target_pose_3.position.y = target_pose_2.position.y 
    target_pose_3.position.z = target_pose_2.position.z + 0.05

    # Orientación para D
    q3 = quaternion_from_euler(-0.385, -0.3, 0)
    ...
    if fraction == 1.0:
        ...
        for i, point in enumerate(plan.joint_trajectory.points):
            point.time_from_start = rospy.Duration(0.2 * i)  # Ajusta el tiempo para hardware real
